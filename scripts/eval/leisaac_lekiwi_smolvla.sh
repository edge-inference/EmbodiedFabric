...
TASK="${TASK:-LeIsaac-LeKiwi-CleanupTrash-v0}"
...
VIDEO_DIR="${VIDEO_DIR:-$ROOT_DIR/logs/videos}"
...
POLICY_CHECKPOINT="${POLICY_CHECKPOINT:-francocipollone/smolvla_lekiwi_ball_into_basket}"
POLICY_LANGUAGE_INSTRUCTION="${POLICY_LANGUAGE_INSTRUCTION:-Pick up tissue trash from the floor and throw it into the trash bin.}"

SERVER_LOG="${SERVER_LOG:-$LOG_DIR/leisaac_policy_server.log}"
EVAL_LOG="${EVAL_LOG:-$LOG_DIR/leisaac_lekiwi_eval.log}"
